...
TEST(AdtPca, AllOK) {
...

    {
        ...
        add_adt_pca(handle, 10, 1000);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::adt_pca::validate(handle, 1000, true, latest));
    }
...
TEST(AdtPca, NoOp) {
    ...
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::adt_pca::validate(handle, 100, true, 1000000));
    }
    ...
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::adt_pca::validate(handle, 100, false, latest));
    }
}

void quick_adt_pca_throw(const std::string& path, int num_cells, std::string msg) {
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::adt_pca::validate(handle, num_cells, true, latest);
    }, msg);
}

TEST(AdtPca, ParametersFailed) {
    const std::string path = "TEST_adt_pca.h5";
