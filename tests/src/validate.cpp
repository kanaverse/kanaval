...
TEST(Overall, Basic) {
    ...
    add_quality_control(handle, num_cells, num_samples);
    add_normalization(handle);
    add_feature_selection(handle, num_genes);
    add_pca(handle, 50, num_cells);

    add_neighbor_index(handle);
    add_tsne(handle, num_cells);
    add_umap(handle, num_cells);

    add_choose_clustering(handle);
    add_kmeans_cluster(handle, num_cells, num_clusters);
    add_snn_graph_cluster(handle, num_cells, num_clusters);

    add_marker_detection(handle, num_genes, num_clusters);
    add_cell_labelling(handle, num_clusters);
    add_custom_selections(handle, num_genes);
