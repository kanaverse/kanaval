...
TEST(SingleInputsV2, AllOK) {
    const std::string path = "TEST_inputs.h5";

    // Trying with MatrixMarket
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_single_matrix(handle);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        auto output = kanaval::v2::validate_inputs(handle, true, latest);
    ...
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_single_matrix(handle, "10X");
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        auto output = kanaval::v2::validate_inputs(handle, true, latest);
    ...
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_single_matrix(handle, "H5AD");
    ...
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        auto output = kanaval::v2::validate_inputs(handle, false, latest);
        EXPECT_EQ(output.num_features[0], 1000);
        EXPECT_EQ(output.num_cells, 100);
        EXPECT_EQ(output.num_samples, 1);
    }
...
TEST(SingleInputsV2, MultiModalOK) {
    const std::string path = "TEST_inputs.h5";
...
static void quick_input_throw(const std::string& path, std::string msg) {
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_inputs(handle, true, latest);
    }, msg);
}

TEST(SingleInputsV2, ParametersFail) {
    const std::string path = "TEST_inputs.h5";
    ...
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_inputs(handle, true, 1000000);
    }, "version 1.0");
...
TEST(SingleInputsV2, MatrixMarketFail) {
    const std::string path = "TEST_inputs.h5";
...
TEST(SingleInputsV2, ResultsFail) {
    const std::string path = "TEST_inputs.h5";
...
void quick_input_throw(const std::string& path, std::string msg, int version) {
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_inputs(handle, true, version);
    }, msg);
}

TEST(SingleInputsV2, ResultsOldOk) {
    const std::string path = "TEST_inputs.h5";
    ...
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_inputs(handle, true, 1001000);
    }
    ...
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_inputs(handle, true, 1002000);
    }
}
...
TEST(MultipleInputsV2, AllOK) {
    const std::string path = "TEST_inputs.h5";

    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_multiple_matrices(handle);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        auto output = kanaval::v2::validate_inputs(handle, true, latest);
...
TEST(MultipleInputsV2, ParametersFailed) {
    const std::string path = "TEST_inputs.h5";
...
TEST(MultipleInputsV2, ResultsFail) {
    const std::string path = "TEST_inputs.h5";
...
TEST(MultipleInputsV2, ResultsOldFail) {
    const std::string path = "TEST_inputs.h5";
...
TEST(SingleInputsV2, SubsettingIndices) {
    const std::string path = "TEST_inputs.h5";
...
TEST(SingleInputsV2, SubsettingFieldValues) {
    const std::string path = "TEST_inputs.h5";
...
TEST(SingleInputsV2, SubsettingRanges) {
    const std::string path = "TEST_inputs.h5";
