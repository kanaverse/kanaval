...
#include "kanaval/v2/cell_labelling.hpp"
#include "../utils.h"
#include <iostream>
...
TEST(CellLabellingV2, AllOK) {
    const std::string path = "TEST_cell_labelling.h5";

    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_cell_labelling(handle);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_cell_labelling(handle, 3));
    }
...
void quick_label_throw(const std::string& path, int num_clusters, std::string msg) {
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_cell_labelling(handle, num_clusters);
    }, msg);
}

TEST(CellLabellingV2, ParametersFailed) {
    const std::string path = "TEST_cell_labelling.h5";
...
TEST(CellLabellingV2, ResultsFailed) {
    const std::string path = "TEST_cell_labelling.h5";
