...
#include "kanaval/v2/custom_selections.hpp"
#include "../utils.h"
#include <numeric>
...
TEST(CustomSelectionsV2, AllOK) {
    const std::string path = "TEST_custom_selections.h5";

    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_custom_selections(handle, 1000);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_custom_selections(handle, 10, { "RNA" }, { 1000 }, latest));
    }
}

TEST(CustomSelectionsV2, MultiModalOK) {
    const std::string path = "TEST_custom_selections.h5";

    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_custom_selections(handle, { "RNA", "ADT" }, { 1000, 5 });
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_custom_selections(handle, 10, { "RNA", "ADT" }, { 1000, 5 }, latest));
    }

    // Fails if multiple modalities were expected but not found.
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_custom_selections(handle, 100, 5);
    }
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_custom_selections(handle, 10, { "RNA", "ADT" }, { 100, 5 }, latest);
    }, "ADT");
}
...
TEST(CustomSelectionsV2, ParametersFailed) {
    const std::string path = "TEST_custom_selections.h5";

    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_custom_selections(handle, 500);
...
TEST(CustomSelectionsV2, ResultsFailed) {
    const std::string path = "TEST_custom_selections.h5";
...
TEST(CustomSelectionsV2, LegacyOK) {
    const std::string path = "TEST_custom_selections.h5";

    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_custom_selections_legacy(handle, 100);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_custom_selections(handle, 10, {}, {100}, 1001000));
    }
}
