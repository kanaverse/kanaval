...
#include "kanaval/v2/snn_graph_cluster.hpp"
#include "../utils.h"
#include <iostream>
...
TEST(SnnGraphClusterV2, AllOK) {
    const std::string path = "TEST_snn_graph_cluster.h5";

    // Basic case.
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_snn_graph_cluster(handle, 1000);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_snn_graph_cluster(handle, 1000));
    }
    ...
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_snn_graph_cluster(handle, 1000, false));
    }
}
...
static void quick_snn_graph_throw(const std::string& path, int num_cells, std::string msg) {
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_snn_graph_cluster(handle, num_cells);
    }, msg);
}

TEST(SnnGraphClusterV2, ParametersFailed) {
    const std::string path = "TEST_snn_graph_cluster.h5";
...
TEST(SnnGraphClusterV2, ResultsFailed) {
    const std::string path = "TEST_snn_graph_cluster.h5";
