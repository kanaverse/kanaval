...
#include "kanaval/v2/batch_correction.hpp"
#include "../utils.h"
#include <iostream>
...
TEST(BatchCorrectionV2, AllOK) {
    const std::string path = "TEST_batch_correction.h5";

    // Works in simple mode.
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_batch_correction(handle, 100, 10);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_batch_correction(handle, 10, 100, 2, latest));
    }
    ...
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_batch_correction(handle, 10, 100, 1, latest));
    }
...
static void quick_correct_throw(const std::string& path, int num_pcs, int num_cells, std::string msg) {
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_batch_correction(handle, num_pcs, num_cells, 2, latest);
    }, msg);
}

TEST(BatchCorrectionV2, ParametersFailed) {
    const std::string path = "TEST_batch_correction.h5";
...
TEST(BatchCorrectionV2, ResultsFailed) {
    const std::string path = "TEST_batch_correction.h5";
