...
#include "kanaval/v2/combine_embeddings.hpp"
#include "../utils.h"
#include <iostream>
...
TEST(CombineEmbeddingsV2, AllOK) {
    const std::string path = "TEST_combine_embeddings.h5";

    // Works in simple mode.
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_combine_embeddings(handle, 100, 10);
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_combine_embeddings(handle, 100, { "RNA", "ADT" }, 10, latest));
    }

    // Works if there's only one modality and no PCs.
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_combine_embeddings(handle, 100, 5);
        handle.unlink("combine_embeddings/results/combined");
    }
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_combine_embeddings(handle, 100, { "RNA" }, 5, latest));
    }

    // Works if the modalities are weighted.
    {
        H5::H5File handle(path, H5F_ACC_TRUNC);
        v2::add_combine_embeddings(handle, 100, 11);
    ...
    {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        EXPECT_NO_THROW(kanaval::v2::validate_combine_embeddings(handle, 100, { "RNA", "ADT" }, 11, latest));
    }
}

static void quick_combine_throw(const std::string& path, int num_cells, const std::vector<std::string>& modalities, int num_pcs, std::string msg) {
    quick_throw([&]() -> void {
        H5::H5File handle(path, H5F_ACC_RDONLY);
        kanaval::v2::validate_combine_embeddings(handle, num_cells, modalities, num_pcs, latest);
    }, msg);
}

TEST(CombineEmbeddingsV2, ParametersFailed) {
    const std::string path = "TEST_combine_embeddings.h5";
...
TEST(CombineEmbeddingsV2, ResultsFailed) {
    const std::string path = "TEST_combine_embeddings.h5";
