 ...
 * @cond
 */
inline int validate_results(const H5::Group& qhandle, int num_cells) {
    auto rhandle = qhandle.open("results");

    int remaining = -1;
    if (rhandle.exists("discards")) {
 ...
 *   
 * Alternatively, `discards` may be absent, in which case the discard dataset is implicitly defined as the `discards` from the upstream `quality_control` groups,
 ...
 */
inline int validate(const H5::H5File& handle, int num_cells, int version) {
    ...
    int remaining = 0;
    try {
        remaining = validate_results(qhandle, num_cells);
